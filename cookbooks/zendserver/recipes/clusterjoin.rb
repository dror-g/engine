...
db_name 	 = nore[:zendserver][:dbname]
...
join_command = "#{node[:zendserver][:zsmanage]} cluster-add-to-server -N #{key_name} -K #{key_secret} -n #{node['hostname']} -i #{node['ipaddress']} -o #{db_host} -u #{db_user} -p #{db_pass} -d #{db_name} -s"
